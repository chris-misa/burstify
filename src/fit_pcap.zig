...
pub fn main() !void {
    if (std.os.argv.len != 2 and std.os.argv.len != 5) {
        std.debug.print("Usage: {s} <pcap file> [<on/off times output file> <burst size output file> <number of bursts output file>]\n", .{std.os.argv[0]});
        std.process.exit(0);
    }
    const filepath = std.mem.span(std.os.argv[1]);
    const bursts_outfile = if (std.os.argv.len == 5) std.mem.span(std.os.argv[2]) else null;
    const flow_size_outfile = if (std.os.argv.len == 5) std.mem.span(std.os.argv[3]) else null;
    const burst_counts_outfile = if (std.os.argv.len == 5) std.mem.span(std.os.argv[4]) else null;
    ...
    // Output number of bursts per flow
    if (burst_counts_outfile) |outfile_name| {
        std.debug.print("Writing burst counts to {s}\n", .{outfile_name});
        const outfile = try std.fs.cwd().createFile(outfile_name, .{});
        defer outfile.close();
        const out = outfile.writer();
        try out.print("bursts\n", .{});
        var it = flows.flows.valueIterator();
        while (it.next()) |bursts| {
            try out.print("{d}\n", .{bursts.items.len});
        }
